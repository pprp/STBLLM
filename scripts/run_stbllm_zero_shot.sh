...
LOG_PATH=./logs/stbllm_eval_zero_shot
    ...
    for gpu in "${GPUS[@]}"; do
        ...
        if [[ -z "${gpu_to_pid[$gpu]}" ]] || ! kill -0 "${gpu_to_pid[$gpu]}" 2>/dev/null; then
            ...
            CUDA_VISIBLE_DEVICES=$gpu python3 run.py /path/to/hf-model/${MODEL_NAME} c4 braq --blocksize 128 \
                ...
                --eval_zero_shot \
                --sparsity_type ${SPARSITY_TYPE} > $LOG_PATH/zero-shot-stbllm_${MODEL_NAME}_c4_ria-structure-${SPARSITY_TYPE}-${SPARSITY_RATIO}-wrec_hessian-4mask-billm-160_main.log 2>&1 &
