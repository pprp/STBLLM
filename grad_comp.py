...
from transformers import AdamW, AutoModelForCausalLM, AutoTokenizer, LlamaTokenizer
...
class ActivationComputation:
    ...
    def update_activation(self):
        for name, activation in self.activations.items():
            self.activations_l1[name] = (
                torch.abs(activation).mean(dim=0, keepdim=True).type(torch.float16)
            )
            self.activations_l2[name] = (activation**2).mean(dim=0, keepdim=True)
...
if __name__ == "__main__":
    ...
    dataloader, _ = get_loaders(
        "wikitext2", nsamples=nsamples, seed=seed, seqlen=2048, tokenizer=tokenizer
    )
    ...
    if args.task == "gradient":
        ...
        with open(
            f"./gradients/llama{args.llama_version}/gradients_aggregrate_norm_l2_model_{model_name}.pth",
            "wb",
        ) as f:
            torch.save(computer.gradients_l2, f)
        with open(
            f"./gradients/llama{args.llama_version}/gradients_aggregrate_norm_l1_model_{model_name}.pth",
            "wb",
        ) as f:
