...
from utils.quant import GPTQQuantizer, HighQuantizer, LowQuantizer
            ...
            if prune_n != 0:
                ...
                for ii in range(W_metric.shape[1]):
                    if ii % prune_m == 0:
                        ...
                        W_mask.scatter_(
                            1,
                            ii + torch.topk(tmp, prune_n, dim=1,
                                            largest=False)[1],
                            True,
                        )
            else:
    ...
    for i in range(len(layers)):
        ...
        for name in subset:
            wrapped_layers[name] = WrappedGPT(
                args, subset[name], layer_name=name)
...
def prune_gblm(
    ...
    for i in range(len(layers)):
        ...
        if (
            ...
            dev = model.hf_device_map[f'model.layers.{i}']
            inps, outs, position_ids = inps.to(
                dev), outs.to(dev), position_ids.to(dev)
...
def prune_ri(
    ...
    for i in range(len(layers)):
        ...
        for name in subset:
            ...
            R = W_abs / (
                sum_abs_cols + sum_abs_rows - W_abs
            )  # Subtract W_abs to avoid counting the weight itself twice
                ...
                for ii in range(R.shape[1]):
                    if ii % prune_m == 0:
                        ...
                        W_mask.scatter_(
                            1,
                            ii
                            + torch.topk(tmp, prune_n, dim=1, largest=True)[
                                1
                            ],  # Change largest to True to prune least relevant weights
                            True,
                        )
            else:
                thresh = torch.sort(R.flatten())[0][
                    int(R.numel() * args.sparsity_ratio)
                ].cpu()
                W_mask = (
                    R <= thresh
                )  # Prune weights with relevance index less than or equal to the threshold
        ...
        for name in subset:
            ...
            if args.prune_method == 'wanda':
                ...
                W_metric = (
                    ...
                    + torch.abs(W) / torch.sum(torch.abs(W),
                                               dim=1).reshape(-1, 1)
                ) * (
                    torch.sqrt(
                        wrapped_layers[name].scaler_row.reshape((1, -1)))
                ) ** args.a
                W_metric = fun1_standardize(
                    W_metric
                )  # NOTE THAT HERE WE CHANGE SOMETHING
            W_mask = (
                torch.zeros_like(W_metric) == 1
            )  # initialize a mask to be all False
            if prune_n != 0:
                ...
                if args.reconstruction:
                    wrapped_layers[name].fasterprune(
                        args.sparsity_ratio, mask=W_mask)
                else:
                    subset[name].weight.data[W_mask] = 0  # set weights to zero
                ...
                for ii in range(RIA.shape[1]):
                    if ii % prune_m == 0:
                        ...
                        W_mask.scatter_(
                            ...
                            + torch.topk(tmp, prune_n, dim=1, largest=True)[
                                1
                            ],  # Pruning the least relevant weights
                            True,
                        )
            else:
                thresh = torch.sort(RIA.flatten())[0][
                    int(RIA.numel() * args.sparsity_ratio)
                ].cpu()
                W_mask = RIA <= thresh
...
def prune_ablate(args, model, tokenizer, dev, prune_n=0, prune_m=0):
    ...
    for i in range(len(layers)):
        ...
        for name in subset:
            gpts[name] = AblateGPT(
                subset[name], gradient_path=args.gradient_path)
